...
module.exports = function (app, hexo) {
  ...
  use('qiniu/getToken', function(req, res, next) {
    ...
    qiniu.conf.ACCESS_KEY = hexo.config.admin.qiniuCfg.AccessKey;
    qiniu.conf.SECRET_KEY = hexo.config.admin.qiniuCfg.SecretKey;

    //bucket is the qiniu's save space name, create by yourself
    bucket = hexo.config.admin.qiniuCfg.BucketName;

    //key is the fileName upload qiniu server
    key = req.body.data;
